"""Modal H100 runner that exports a calibration-refreshed SVA artifact."""
...
app = modal.App("sva-export-refreshed-artifact-h100")
...
REMOTE_ARTIFACT_DIR = "/artifacts/sva-smollm2-135m-2x256-longctx-refresh-v1"
...
ARGS = [
    ...
    REMOTE_ARTIFACT_DIR,
    "--profile-name",
    "sva-smollm2-135m-2x256-longctx-refresh-v1",
    "--calibration-length",
    "32768",
...
def export_refreshed_artifact() -> str:
    import os
    import subprocess
    import sys
    ...
    print("refreshed_artifact_h100_start", flush=True)
    ...
    print(f"refreshed_artifact_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Refreshed SVA artifact export failed with exit code {return_code}")
    volume.commit()
    print(f"refreshed_artifact_remote_dir,{REMOTE_ARTIFACT_DIR}", flush=True)
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = export_refreshed_artifact.spawn()
    print(f"function_call_id,{call.object_id}")
    print(f"dashboard,{call.get_dashboard_url()}")
    print(f"remote_artifact_dir,{REMOTE_ARTIFACT_DIR}")
    print(
        "download_command,"
        "modal volume get sva-artifacts /sva-smollm2-135m-2x256-longctx-refresh-v1 "
