"""Modal H100 runner for late4 passkey robustness checks."""
...
app = modal.App("sva-passkey-late4-robustness-h100")
...
def run_passkey_late4_robustness() -> str:
    import os
    import subprocess
    import sys
    ...
    print("passkey_late4_robustness_h100_start", flush=True)
    ...
    print(f"passkey_late4_robustness_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Late4 robustness benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_passkey_late4_robustness.spawn()
