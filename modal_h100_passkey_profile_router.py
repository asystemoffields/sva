"""Modal H100 runner for passkey language tests with context-routed SVA profiles."""
...
app = modal.App("sva-passkey-profile-router-h100")
...
def run_passkey_profile_router() -> str:
    import os
    import subprocess
    import sys
    ...
    print("passkey_profile_router_h100_start", flush=True)
    ...
    print(f"passkey_profile_router_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Passkey profile-router benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_passkey_profile_router.spawn()
