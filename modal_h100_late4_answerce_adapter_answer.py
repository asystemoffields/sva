"""Modal H100 runner for answer-decode validation of the answer-CE late4 adapter."""
...
app = modal.App("sva-late4-answerce-adapter-answer-h100")
...
def run_late4_answerce_adapter_answer() -> str:
    import os
    import subprocess
    import sys
    ...
    print("late4_answerce_adapter_answer_h100_start", flush=True)
    ...
    print(f"late4_answerce_adapter_answer_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Late4 answer-CE adapter answer benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_late4_answerce_adapter_answer.spawn()
