"""Modal H100 runner for passkey language shortlist/budget scale-out."""
...
app = modal.App("sva-passkey-language-scaleout-h100")
...
def run_passkey_language_scaleout() -> str:
    import os
    import subprocess
    import sys
    ...
    print("passkey_language_scaleout_h100_start", flush=True)
    ...
    print(f"passkey_language_scaleout_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Passkey language scale-out benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_passkey_language_scaleout.spawn()
