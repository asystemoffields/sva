"""Modal H100 runner for passkey language pressure on early-layer SVA sockets."""
...
app = modal.App("sva-passkey-early26-language-h100")
...
def run_passkey_early26_language() -> str:
    import os
    import subprocess
    import sys

    os.chdir("/root/sva")
    print("passkey_early26_language_h100_start", flush=True)
    lines: list[str] = []
    ...
    print("passkey_early26_language_h100_done", flush=True)
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_passkey_early26_language.spawn()
