"""Modal H100 runner for the SVA passkey language benchmark."""
...
app = modal.App("sva-passkey-language-h100")
...
def run_passkey_language() -> str:
    ...
    print("passkey_language_h100_start", flush=True)
    ...
    print(f"passkey_language_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Passkey language benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_passkey_language.spawn()
