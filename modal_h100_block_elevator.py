"""Modal H100 runner for block-first SVA elevator benchmarking."""
...
app = modal.App("sva-block-elevator-h100")
...
def run_block_elevator() -> str:
    import os
    import subprocess
    import sys

    os.chdir("/root/sva")
    cmd = [sys.executable, "-u", "experiments/sva_block_elevator_benchmark.py", *ARGS]
    print("block_elevator_h100_start", flush=True)
    ...
    print(f"block_elevator_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Block elevator benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_block_elevator.spawn()
