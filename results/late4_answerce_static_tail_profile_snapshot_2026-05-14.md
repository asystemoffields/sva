...
Next target: vectorize or chunk the remaining decode catalog path. The most promising path is to cache device-ready SVA projections/codebooks, avoid per-step product-key encoding work where possible, and rebuild or append postings in larger batches instead of touching the full routing structure every generated token.
