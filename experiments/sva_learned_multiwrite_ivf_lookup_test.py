"""Multi-write IVF serving test for the learned SVA compressed ranker."""
...
@torch.no_grad()
def assign_to_multiple_centroids(
    x: torch.Tensor,
    ...
    actual_writes = min(writes, centroids.shape[0])
    ...
    for start in range(0, x_float.shape[0], chunk_size):
        ...
        labels.append(distance.topk(actual_writes, largest=False, dim=-1).indices)
    return torch.cat(labels, dim=0)


def build_multiwrite_buckets(labels: np.ndarray, n_centroids: int) -> list[int]:
...
@torch.no_grad()
def lookup_multiwrite_ivf_candidates(
    q_low: torch.Tensor,
    centroids: torch.Tensor,
    ...
    bucket_sets = [
        build_multiwrite_buckets(eval_labels[head_idx].cpu().numpy(), n_centroids)
...
def print_multiwrite_result(
    ...
    print(
        "learned_multiwrite_ivf_result,"
        f"{layer_idx},{seq_len},{target_context},{rank_dim},{centroids},{writes},{probes},{budget},"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Serve learned SVA ranker with multi-write IVF routing.")
    ...
    parser.add_argument("--centroids", default="2048,4096")
    ...
    print(
        "learned_multiwrite_ivf_header,"
        "layer,seq_len,target_context,rank_dim,centroids,writes,probes,budget,kmeans_iters,train_steps,final_loss,"
            ...
            for writes in write_values:
                if writes == 1:
                    eval_labels = torch.stack(
                        [
                            assign_to_centroids(eval_k_low[head_idx], centroids[head_idx], args.assign_chunk_size)[:, None]
                ...
                print(f"progress,writes_ready,{layer_idx},{actual_centroids},{writes}", flush=True)

                for probes in probe_values:
                    candidate_records, actual, million = lookup_multiwrite_ivf_candidates(
                        eval_q_low,
                        centroids,
                        eval_labels,
                        eval_positions,
                    ...
                    print(
                        f"progress,lookup_ready,{layer_idx},{actual_centroids},{writes},{probes},"
                    ...
                    for budget in budgets:
                        ...
                        key_tuple = (actual_centroids, writes, probes, budget)
    ...
    for (centroids_count, writes, probes, budget), bucket in sorted(aggregate.items()):
        print_multiwrite_result(
            "all",
            eval_seq_len,
            args.target_context,
