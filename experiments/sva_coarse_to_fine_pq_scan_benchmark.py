"""Synthetic throughput benchmark for staged coarse-to-fine PQ scans."""
...
def pq_shortlist_scores(
    tables: torch.Tensor,
    codes: torch.Tensor,
    shortlist_idx: torch.Tensor,
) -> torch.Tensor:
    heads, queries, shortlist = shortlist_idx.shape
    subspaces = tables.shape[2]
    context = codes.shape[1]
    scores = torch.zeros(heads, queries, shortlist, device=tables.device, dtype=torch.float32)

    for subspace_idx in range(subspaces):
        sub_codes = codes[:, :, subspace_idx]
        selected_codes = sub_codes[:, None, :].expand(heads, queries, context).gather(
            dim=-1,
            index=shortlist_idx,
        )
        scores += tables[:, :, subspace_idx, :].float().gather(
            dim=-1,
            index=selected_codes.long(),
        )
...
def benchmark_config(
    ...
    fine_tables = make_tables(heads, queries, fine_subspaces, fine_codewords, device, dtype)
    coarse_codes = make_codes(heads, context, coarse_subspaces, coarse_codewords, device)
    fine_codes = make_codes(heads, context, fine_subspaces, fine_codewords, device)
    ...
    for _ in range(warmup):
        ...
        fine_scores = pq_shortlist_scores(fine_tables, fine_codes, coarse_idx)
        _ = fine_scores.topk(budget, dim=-1).indices
    synchronize(device)

    coarse_scan_times: list[float] = []
    shortlist_topk_times: list[float] = []
    fine_rescore_times: list[float] = []
    ...
    for _ in range(repeats):
        ...
        fine_rescore_times.append((after_fine - after_shortlist) * 1000.0)
        verify_topk_times.append((after_verify - after_fine) * 1000.0)
        total_times.append((after_verify - start) * 1000.0)

    coarse_bits_per_key = coarse_subspaces * max(1, (coarse_codewords - 1).bit_length())
    fine_bits_per_key = fine_subspaces * max(1, (fine_codewords - 1).bit_length())
    coarse_code_lookups = heads * queries * context * coarse_subspaces
    fine_code_lookups = heads * queries * shortlist * fine_subspaces
    full_fine_code_lookups = heads * queries * context * fine_subspaces
    print(
        "coarse_to_fine_pq_scan_result,"
        f"{context},{heads},{queries},"
        f"{coarse_subspaces},{coarse_codewords},{coarse_bits_per_key},"
        f"{fine_subspaces},{fine_codewords},{fine_bits_per_key},"
        f"{shortlist},{budget},"
        f"{coarse_code_lookups},{fine_code_lookups},{full_fine_code_lookups},"
        ...
        f"{average(fine_rescore_times):.3f},{percentile(fine_rescore_times, 50):.3f},{percentile(fine_rescore_times, 95):.3f},"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark staged coarse-to-fine PQ throughput.")
    ...
    print(f"fine_configs,{args.fine_configs}")
    ...
    print(f"dtype,{dtype}")
    print(
        "coarse_to_fine_pq_scan_header,"
        "context,heads,queries,"
        "coarse_subspaces,coarse_codewords,coarse_bits_per_key,"
        "fine_subspaces,fine_codewords,fine_bits_per_key,"
        "shortlist,budget,"
        "coarse_code_lookups,fine_code_lookups,full_fine_code_lookups,"
        ...
        "fine_rescore_ms_avg,fine_rescore_ms_p50,fine_rescore_ms_p95,"
    ...
    for queries in comma_ints(args.queries):
        for coarse_subspaces, coarse_codewords in pq_configs(args.coarse_configs):
            for fine_subspaces, fine_codewords in pq_configs(args.fine_configs):
