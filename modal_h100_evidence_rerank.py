"""Modal H100 runner for evidence-aware SVA rerank benchmarking."""
...
app = modal.App("sva-evidence-rerank-h100")
...
def run_evidence_rerank() -> str:
    ...
    print("evidence_rerank_h100_start", flush=True)
    ...
    print(f"evidence_rerank_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Evidence rerank benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_evidence_rerank.spawn()
