"""Modal H100 runner for a larger held-out answer panel on the answer-KL+CE late4 adapter."""
...
app = modal.App("sva-late4-answerce-broad-panel-h100")
...
@app.function(image=image, gpu="H100", timeout=180 * 60, volumes={"/artifacts": volume})
def run_late4_answerce_broad_panel() -> str:
    import os
    import subprocess
    import sys
    ...
    print("late4_answerce_broad_panel_h100_start", flush=True)
    ...
    print(f"late4_answerce_broad_panel_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Late4 answer-CE broad panel failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_late4_answerce_broad_panel.spawn()
