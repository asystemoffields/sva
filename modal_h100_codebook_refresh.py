"""Modal H100 runner for held-out SVA codebook refresh benchmarking."""
...
app = modal.App("sva-codebook-refresh-h100")
...
ARGS = [
    ...
    "0,15,29",
    "--variants",
    "artifact_identity,calib_identity,calib_hadamard,calib_signed_hadamard,eval_refit_identity",
    "--budgets",
    "512,1024,2048",
    "--topk",
    "16",
...
def run_codebook_refresh() -> str:
    ...
    print("codebook_refresh_h100_start", flush=True)
    ...
    print(f"codebook_refresh_h100_exit,{return_code}", flush=True)
    if return_code != 0:
        raise RuntimeError(f"Codebook refresh benchmark failed with exit code {return_code}")
    return "".join(lines)


@app.local_entrypoint()
def main() -> str:
    call = run_codebook_refresh.spawn()
